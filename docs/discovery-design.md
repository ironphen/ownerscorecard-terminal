...
DISQUALIFICATIONS (cheap checks, named)
  ...
  /withheld                   the publication's own basket: records IT could not read, each with
                              its reason. Never called "too hard" — the too-hard pile is the
                              reader's, personal by definition, and lives only in the gauntlet's
                              private marks (feature 15)
...
Sitewide it opens a uniformly random company page; on a chapter, a random member. Uniform, withheld-reads records included: any weighting toward test-passers would be an implicit ranking. Data: the existing `company-index.json.js` slug index and a few lines of client-side JS. Size: S.
...
11. Withheld reads (/withheld) — REFRAMED 2026-07-09, on the owner's correction
The original design called this the Too-Hard Pile, and that was a category error against the framework it borrowed from. Munger's three baskets — in, out, too tough — are the INVESTOR'S baskets: the too-hard call is an act of knowing where one's own competence ends, and everyone's pile is different. Tech was too hard for Buffett in 1999 and legible to an engineer the same year. A publication that prints a too-hard list has made the one judgment nobody can make for another person — a verdict about the reader's circle of competence, which this site must leave to the reader along with the price.
What the publication CAN honestly publish is its own limitation: records it could not read reliably from the filings. Same page, same data, different subject — the sentence is about us, never about the reader or the business's comprehensibility. Alphabetical, because absence of a read has no magnitude to sort. Each row states the specific reason: record shorter than four readable years; balance sheet incoherent, current assets exceed total assets; ADR ratio unverified, per-share reads withheld; financial company, operating reads are category errors; restatement in progress, prior years unreliable. Member company pages get a plain strip: "Withheld. This publication could not read this record reliably from the filings; these reads were not run:" followed by the withheld reads. Withheld records never appear on archetype pages as failing; they appear in their own basket, and in the manual and the chapters, which are reading surfaces, not tests. The words "too hard" never appear on this surface. Data: the withhold paths already coded (the lenses' null-tolerance guards, the short-record checks, the NCAV incoherence checks, the margin withholds, `adrBasis.mjs` warn mode, `freshness.mjs` staleness) currently discard their reasons; collect them into a per-company readability record at build time. Size: M.

12. The Unflagged (/archetypes/unflagged, new)
Membership: at least eight readable years, zero fired red-flag items, zero flagged inversion checks, and no withheld-reads record. Alphabetical only, deliberately unsortable. Each row: ticker, name, "0 of N disqualifiers fired," years readable. The header states what it is: the pile worth expensive attention because the cheap attention found nothing. Its smallness is the point. Never counted by confluence (section 6 explains). Data: the red-flags lens plus `inversion.mjs` `inversionChecks()` run universe-wide, one extra pass inside the memoized `computeLenses()` build (inversion currently runs only on company pages), plus the readability record from feature 11. Size: M.
...
14. The case against, first (strip on /c/[ticker] and /jp/[ticker]) — BUILT, THEN REMOVED BY THE OWNER (2026-07-09)
REMOVED after shipping, on the owner's judgment, and the reason is doctrine: every line in the strip was factual, but a standing section titled "The case against" at the top of every company page is a pronouncement BY ARRANGEMENT — the publication prosecuting the business before the reader has seen the record. ValueLine's tables never editorialized their order; Moody's led with the record; Graham handed the reader tests to run, not conclusions pre-run at the masthead. The disqualification layer survives where the reader opens it deliberately: the /flags registers, /withheld, the inversion and Graham panels in their own sections down the page. Do not rebuild this surface. (The original spec follows for the record.)
Position: directly under the identity header (ticker, name, lede) and before segments and prose. The reader must know what a thing is before evidence against it means anything; after that, the disconfirming evidence comes before the story does its seducing. One line per fired item, each with its figure, each anchoring to its full panel below: fired red flags; flagged inversion checks (margin fade, dilution, leverage, conversion, working capital, write-offs); Graham tests failed, stated as n of testable; a candor low if present; withheld reads once the readability record exists. Companies with a withheld-reads record show the withheld strip here instead (the publication's limitation, stated in the first person — never "too hard"). When nothing fired: "No disqualifier fired on the cheap checks. The two tests that remain, understanding and price, are yours." Never a green badge. Data: a reordering summary over outputs the page build already holds: `inversionChecks()`, red-flags membership, `grahamTests()`, the candor reads. No new computation. Size: S.

15. The gauntlet (client-side on every company page; private record at /my/gauntlet)
"Run the gauntlet" opens a sequence over the page's existing build JSON (`c/[ticker].json.js`), gates in the 2007-letter order, with most walks ending early by design; that is the method working. NOTE (2026-07-09): the standing case-against page strip was removed by the owner (feature 14 above) — inside a gauntlet the READER chose to run, showing the fired flags as a gate is different in kind (the reader asked for the walk), but the gate must be re-reviewed with the owner before milestone 3 builds it, and it must not resurrect a page-level strip. Gate 0, disqualifications: every fired flag and inversion check up front, with "the walk usually ends here" when a grave flag (restatement, material weakness) fired. Gate 1, understanding: segments, revenue mix, the business description; the reader marks understand, don't, or too hard. Gate 2, economics: median operating margin through the cycle, ROIC years, gross profitability, the capital-light read, the ten-year table. Gate 3, stewardship: return on each retained dollar, buyback dollars against the share-count trend, the dividend record, the candor read, integrity flags. Gate 4, price, which is yours: the page hands over owner earnings per share, NCAV per share, and the reverse-DCF inputs, and stops; no gate is computed for it. The reader marks each gate and the walk ends where a gate is marked failed. Marks live in localStorage; /my/gauntlet is a static shell, client-rendered from localStorage, listing every company walked and the gate where each died, with dates. Never published, never aggregated. Data: existing reads only: `segments.mjs`, `durability.mjs`, `capital.mjs`, `graham.mjs`, the candor and integrity langMap, `inversion.mjs`, `valuationInputs.mjs`, `reverseDcf.mjs`, stewardship facts. New work is one client component. Size: L.
...
- The case against, first (14) — shipped in this milestone, then REMOVED by the owner the same day (see feature 14 for the reason; do not rebuild)
...
- The shelves (4)
- Withheld reads (11), here because the manual's claim of completeness requires the unreadable be labeled, not hidden
...
Complete on its own: both of Graham's nets published; list membership shows its persistence; the publication's baskets exist (the tests met, the flags fired, the records it could not read) while the reader's own three baskets — in, out, too tough — live only in the reader's private gauntlet marks; the reader's own discipline has a surface, all of it private.
...
- Weighted or featured randomness. Weighting toward test-passers is an implicit ranking. Open anywhere is uniform, withheld-reads records included.
- A published too-hard pile (RATIFIED 2026-07-09). Everyone's too-hard pile is different: Munger's three baskets are the investor's own, and the too-hard call is an act of knowing where one's OWN competence ends. A publication printing a too-hard list makes a judgment about the reader's circle of competence — the one judgment the framework reserves to the reader, alongside the price. The site publishes its own limitation instead (withheld reads: "this publication could not read this record reliably"), and the reader's too-hard verdict lives only in the private gauntlet marks. The words "too hard" designate no company anywhere on the site; the concept appears only where the discipline itself is taught as the reader's.
